...
topic = st.sidebar.selectbox('Topics',('Select a topic','Language Models', 'NLP Tasks', 'NLP Applications', 'Tools and Resources'))

if topic == 'Select a topic':
    st.text('')
    st.write('This app introduces different NLP tasks and their proposed solutions. There are many resources explaining the theory online, so this app will be mainly comparing pros and cons of different models and solutions. Which model to use really depends on your size of dataset, speed reguirement, cost and many other factors. Like Bert can be directly or indirectly used in all the tasks frome sentence level to the documents level in this overview, it is not always the best solution.')
    st.text('')
    st.subheader('You can check the modern language models and their applications or just select the NLP task you are interested in.')
...
if topic == 'Language Models':
    st.header('Language Models')
    
    models = ['Choose a model', 'RNN/LSTM/GRU','Elmo', 'Transformer', 'Bert', 'GPT/GPT-2']
    model = st.sidebar.selectbox('Language models', models)

    if model == 'Choose a model':

        st.write('The advantage of a pretrained model it already memorizes a lot text with context in the model. When you have rather small dataset, fine-tuning the pretrained model with your dataset')
        # st.write('The advantage of pretrained model is it embed In real world application,The history of language model starts from one hot embedding, followed by word embedding')
        # st.write('The simple seq-2-seg model is based on RNN. Th simple RNN structure is as following')
    if model == 'Transformer':
        st.image('tf.png', caption='From ‘Attention Is All You Need’ by Vaswani et al.', use_column_width=True)

    if model == 'Bert':
        st.write('Bert is the encoder part of the transformer.')
        st.write('Pretaining task for Bert are Masked Language Model(MLM) and Next Sentence Prediction(NSP)')
        st.write('The model architecture and the keras code are showing below:')

    
        
        st.write('Since Bert is the encoder part of the transformer, it is more suitable for NLU related tasks')


        st.subheader('How to fine-tune Bert?')
        st.write('I believe everyone has seen this picture for many times')
        st.image('ftb.png', caption='source: https://arxiv.org/pdf/1810.04805.pdf')


        st.write('Here we use fake news dataset as an example.')




        st.subheader('Bert for Word Embedding')



        st.subheader('Bert with Knowledge Graph')

        st.write('Knowledge graph can be used to train Bert and then used to build knowledge graph from unctructured text.')


        st.subheader('Multi-Task Bert')


        st.write('Most tasks uses each Bert model for different tasks, but there are research also train the model with multiple tasks.')

        st.subheader('Distallation and Compression of Bert')



        st.subheader('Further reading and resources')
        st.markdown('[awesome-bert](https://github.com/Jiakui/awesome-bert)')
        st.markdown('[BertViz](https://github.com/jessevig/bertviz)')

    if model == 'GPT/GPT-2':
        st.write('GPT-2 is the decoder part of the transformer. Therefore, it is better for natural language generation')
...
elif topic == 'NLP Tasks':


    tasks = ['Choose a task', 'Part of Speech Tagging',  'Dependency Parsing', 'Named Entity Recognition', 'Textual Entailment', 'Text Classification', 'Text Similarity','Sentiment Analysis', 'Ranking','Question Answering']
    #often used tools just list in each tasks
    task = st.sidebar.selectbox('NLP tasks', tasks)

    if task == 'Choose a task':
...
elif topic == 'NLP Applications':

    tasks = ['Choose an application', 'Machine Translation', 'Automatic Summarization', 'Dialogue System', 'Recommendation System']
    #often used tools just list in each tasks
    task = st.sidebar.selectbox('NLP applications', tasks)
...
elif topic == 'Tools and Resources':
    ...
    st.markdown('[Bert as Service]()')

    st.subheader('Training')

    st.markdown('[FARM](https://github.com/deepset-ai/FARM)')

    st.markdown('A very useful framework to train your NLP models for transfer learning.')
    ...
    st.subheader('Visualization and Debugging')

    st.markdown('[Language Interpretability Tool(LIT) from Google PAIR](https://github.com/pair-code/lit)')

    st.markdown('A very useful visualization tool to inspect and compare your models and datapoints.') 
...
# if model == 'Dependecy Parsing':
#     st.header('Dialog System')
#     st.text('Dialog system is also called conversational AI')
#     st.image('rasa.png')
